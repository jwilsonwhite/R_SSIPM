...
R.mean <- log(5) # recruitment
R.sd <- 1
Im.mean <- log(5) #immigration
Im.sd <- 1
...
Names <- c(paste0("r",c(0:(dim(fishtotal)[2]))), 'Im0','Im1','Im2','Im3','F','error') # for now assume one burn-in recruitment rate, three years of observations, and one harvest rate, and one process error term

Means <- c(rep(R.mean,dim(fishtotal)[2]+1), rep(Im.mean,4) ,F.mean,error.mean) #can't have negative fishing or recruitment
SDs <- c(rep(R.sd,dim(fishtotal)[2]+1), rep(Im.sd,4),F.sd,error.sd)
# Type <- rep('lognormal', length(Means))
Type <- c(rep('lognormal',dim(fishtotal)[2]+1), rep('lognormal',4),
...
ggplot()+
  ...
  ylim(min(Post.fit$PosteriorProb[c(0:length(Post.fit$PosteriorProb))]),max(Post.fit$PosteriorProb))
...
#       im.ipm <- run.IPM(fix.param, im.cand.param, im.data, burnin.im = FALSE, ipm.im = FALSE, burnin = FALSE)
#       stbstate <- im.ipm$N[,50] # Distant future based on IPM
#       stbstate <- stbstate/sum(stbstate*fix.param$dx)
...
# #graph
# p.oxca <-  ggplot()+
#   geom_density(bk_size %>% filter( species_code%in% "OXCA")%>%
...
```{r}
# recruit <- songstotal %>% filter(total_length<=8,
# species_code == "SEPU" ) %>% 
...
# geom_density(test.bk, mapping = aes( x = total_length), color = 'red')+
...
# 
# Names <- c(paste0("r",c(0:(dim(fishtotal)[2]))), 'Im0','Im1','Im2','Im3','F','error') # for now assume one burn-in recruitment rate, three years of observations, and one harvest rate, and one process error term
# Means <- c(rep(1,dim(fishtotal)[2]+1), rep(2,4), 0.25, 0.1)
# SDs <- c(rep(R.sd,dim(fishtotal)[2]+1), rep(Im.sd,4), F.sd,1)
# # Type <- rep('lognormal', length(Means))
...
#                names_to = "year",
#                values_to = "N")
# tipm.na <- sizedist.na %>% select(c(1:23)) %>%
#   rename_all(~y) %>%  #rename columns to match year
#   add_column(size = c(1:nrow(sizedist)), .before = '2000') %>%
#   mutate(size = size*fix.param$dx) %>%
#   pivot_longer(cols = "2000":"2022",
...
#                values_to = "N")
# ggplot()+
#   geom_line(data = tipm, aes(x = size, y = N/5), color = "red")+
#   geom_line(data = tipm.na,aes(x = size, y = N/5), color = "gold")+
#   geom_density(data = tfish, aes(x = size)) +
#   facet_wrap(~year, scales = "free_y") +
#   scale_y_continuous("survey density", sec.axis = sec_axis(~.*5,
#                                          "ipm density"))+
