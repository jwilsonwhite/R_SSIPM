...
# phase <- c(c(1:dim(fishtotal)[2]), rep("2",dim(fishtotal)[2]-7))
# phase <- c(rep("1",9))
...
R.sd <- 0.1
Im.mean <- log(0.01) #immigration
Im.sd <- 0.1
F.mean <- log(0.25) # fishing
F.sd <- 0.1
error.mean = 1.5 # process error shape
error.sd = 0.025 # process error scale


Names <- c(paste0("r",c(1:(dim(fishtotal)[2]))), paste0("Im",c(1:2)), 'F')
# Names <- c( paste0("Im",c(1:2)), 'F')
Means <- c(rep(R.mean,dim(fishtotal)[2]), log(0.03), Im.mean, F.mean)
# Means <- c( log(0.03), Im.mean, F.mean)
SDs <- c(rep(R.sd,dim(fishtotal)[2]), rep(Im.sd,2), F.sd)
# SDs <- c( rep(Im.sd,2), F.sd)
Type <- c( c(rep("lognormal",dim(fishtotal)[2]),rep("lognormal",2), "lognormal")) #type for recruitment, immigration, fishing, and process error
# Type <- c(rep("lognormal",2), "lognormal")
...
Fit <- fit.SSIPM.MCMC(fix.param,fishtotal,Prior,'mc_str_ar_A.RData', burnin = FALSE, ipm.r = FALSE)
# save the results...
...
#   ggplot()+
#   geom_line(aes(y = Im1, x = iteration))
...
### !!!!!!! TEST MAXIMUM LIKELIHOOD METHOD
```{r}
#prep cand.param for model
# cand.param = exp(Prior$Means)
# names(cand.param) = Prior$Names
# #prep immigration size distribution
...
# m1 <- mle2(minuslogl =  run.IPM, start = list(fix.param = fix.param, cand.param = cand.param), data = list(Data = fishtotal))
# 
# run.IPM(fix.param = fix.param, cand.param = cand.param, burnin = FALSE, burnin.im = FALSE, burnin.r = FALSE, Data = fishtotal)[1]

# , burnin = FALSE, burnin.im = FALSE, burnin.r = FALSE, ipm.im = TRUE,ipm.r = TRUE, Data = fishtotal
```
...
Means <- c(rep(exp(R.mean),dim(fishtotal)[2]), 0.03, 0.02, 0.25)
