...
# Fit.p.a.sepu <- fit.SSIPM.MCMC(fix.param.sepu,fishtotal.sepu,Prior.p.a.sepu,'mc_str_ar_P_A_sepu_noyoy.RData', burnin = FALSE)
# Fit.a.sepu <- fit.SSIPM.MCMC(fix.param.sepu,fishtotal.sepu,Prior.a.sepu,'mc_str_ar_A_sepu_noyoy.RData', burnin = FALSE, ipm.r = FALSE)
# Fit.p.sepu <- fit.SSIPM.MCMC(fix.param.sepu,fishtotal.sepu,Prior.p.sepu,'mc_str_ar_P_sepu_noyoy.RData', burnin = FALSE, ipm.i = FALSE)
# 
# Fit.p.a.sepu.m <- fit.SSIPM.MCMC(fix.param.m.sepu,fishtotal.m.sepu,Prior.p.a.sepu.m,'mc_str_ar_P_A_sepu_mitigation_noyoy.RData', burnin = FALSE)
# Fit.p.sepu.m <- fit.SSIPM.MCMC(fix.param.m.sepu,fishtotal.m.sepu,Prior.p.sepu.m,'mc_str_ar_P_sepu_mitigation_noyoy.RData', burnin = FALSE, ipm.i = FALSE)
# Fit.a.sepu.m <- fit.SSIPM.MCMC(fix.param.m.sepu,fishtotal.m.sepu,Prior.a.sepu.m,'mc_str_ar_A_sepu_mitigation_noyoy.RData', burnin = FALSE, ipm.r = FALSE)
```
...
### California Sheepehad
```{r}
ssipm.dic(postproc.MCMC = Post.fit.p.a.sepu)$DIC #6300.428
ssipm.dic(postproc.MCMC = Post.fit.p.sepu)$DIC # 42128.89
ssipm.dic(postproc.MCMC = Post.fit.a.sepu)$DIC #11285.53
...
# ggsave("post.pred.p.a.sepu.pdf", width = 11, height = 8.5,units = "in",path = here("SSIPM_output","plots"))
...
# ggsave("post.pred.p.a.sepu.m.pdf", width = 11, height = 8.5,units = "in",path = here("SSIPM_output","plots"))
...
# ggsave("post.pred.p.a.pacl.pdf", width = 11, height = 8.5,units = "in",path = here("SSIPM_output","plots"))

pacl.m <- post.pred.check(Data = fishtotal.m.pacl, fix.param = fix.param.m.pacl, Post.fit = Post.fit.p.a.pacl.m, ipm.burnin = FALSE)
# ggsave("post.pred.p.a.pacl.m.pdf", width = 11, height = 8.5,units = "in",path = here("SSIPM_output","plots"))
...
# ggsave("post.pred.p.a.pane.pdf", width = 11, height = 8.5,units = "in",path = here("SSIPM_output","plots"))


pane.m <- post.pred.check(Data = fishtotal.m.pane,fix.param = fix.param.m.pane, Post.fit = Post.fit.p.a.pane.m, ipm.burnin = FALSE)
# ggsave("post.pred.p.a.pane.m.pdf", width = 11, height = 8.5,units = "in",path = here("SSIPM_output","plots"))
...
# ggsave("trace.p.a.sepu.pdf", width = 11, height = 8.5,units = "in",path = here("SSIPM_output","plots"))
...
songs %>% 
  ...
  ggplot()+
  geom_point(aes(x = year, y = count, group = species_code, color = species_code))+
  geom_line(aes(x = year, y = count, group = species_code, color = species_code))+
...
im.data <- left_join(im.fish.data, im.transect.data) %>%  #merge transect and fish data
  ...
  ######---Below is aveage by year, need average by transect x year to get se
# im.data <- songs %>%
#   filter( species_code %in% c("SEPU","PACL","PANE"),
#           reef_code %in% c("SMK",'WNR'),
#          transtype_strata %in% c("BOTTOM",NA),
...
im.data %>% 
  ...
  filter(!(phase_built_code == "M" & year <2008)) %>% #remove points prior to mitigation reef survey
  # filter(total_length>8) %>%  summarise(sum = sum(density), .by = c(year,species_code)) %>% 
  ggplot() + 
  geom_line(aes(x = year, y = average*10000, group = species_code, color = species_code), size = 2)+
  geom_errorbar(aes(y = average*10000, x = year, ymin = low_cl*10000, ymax = up_cl*10000, colour = species_code), size = 0.7)+
  geom_point(aes(x = year, y = average*10000, group = species_code, color = species_code), fill = "black", size = 3)+
  ...
  # scale_color_discrete(labels = c("Kelp Bass", "Barred Sand Bass", "California Sheephead"))+
  scale_color_brewer(palette = "Dark2",labels = c("Kelp Bass", "Barred Sand Bass", "California Sheephead"))
  
################## Median size per year 
# im.mean <- songs %>%
#   filter( species_code %in% c("SEPU","PACL","PANE"),
#           reef_code %in% c("SMK",'WNR'),
#          transtype_strata %in% c("BOTTOM",NA),
...
#                 "N" = "Natural Reef")
# 
# im.mean %>% mutate(year = as.character(year)) %>% mutate(year = as.numeric(year)) %>%  ggplot() + geom_line(aes(x = year, y = mean, group = species_code, color = species_code) )+ 
...
#   facet_wrap(~phase_built_code, labeller = as_labeller(site_names))+
#   theme(axis.text.x = element_text(size = 12), axis.text.y = element_text(size = 12),strip.text.x = element_text(size =12))
```
