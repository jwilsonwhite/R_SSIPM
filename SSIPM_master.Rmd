...
# } #phase 2 & 3 data from public repository
{
  ...
  songs <- songs_1 %>% select(3:18) ########!!!!!!!!!!! Temp. using Phase1 data only
...
phase <- c(1,rep("2",6), rep("3",dim(fishtotal)[2]-7))
...
# Names <- c(paste0("r",c(1:(dim(fishtotal)[2]))), "Im", 'F')
Names <- c(paste0("r",c(1:(dim(fishtotal)[2]))), paste0("Im",c(1:2)),'F')
Means <- c(rep(R.mean,dim(fishtotal)[2]), rep(Im.mean,2), F.mean)
SDs <- c(rep(R.sd,dim(fishtotal)[2]), rep(Im.sd,2), F.sd)
Type <- c( c(rep("lognormal",dim(fishtotal)[2]),rep("lognormal",2),
...
# 
# # Names <- c(paste0("r",c(1:(dim(fishtotal)[2]))), paste0("Im",c(1:2)),'F','error') # for now assume one burn-in recruitment rate, three years of observations, and one harvest rate, and one process error term
...
# 
# Means <- c(
# # 0.0009,0.0009,0.0009,0.0009,0.005,0.0009,0.0009,0.02,0.0009,0.0009,
# # 0.0009,0.0009,0.0009,0.009,0.009,0.02,0.001,0.0009,0.0009,0.0009,
# # 0.001,0.001,0.0009,
# rep(0.0025,dim(fishtotal)[2]),
# 0.05,259,
# # rep(0.021,dim(fishtotal)[2]),
# 
# exp(F.mean))
...
#   pivot_longer(cols = c("2000":"2006"),
#                                     names_to = "year",
#                                     values_to = "count") %>%
#   left_join(., t.sect) %>%
#   mutate(density = count/t.transect.)
# 
# tipm <- sizedist %>% select(c(1:7)) %>% #IPM with data fitting
...
# 
# ggplot()+
#   geom_line(data = tipm, aes(x = size, y = N), color = "blue")+
...
#   geom_line(data = tfish, aes(x = size, y = density)) +
