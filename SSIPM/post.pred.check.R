...
ppc.list <- list()
for (j in (ncol(Data)-2):ncol(Data)) { #get poster predictive check for the last 3 years
...
#standardize and place in data frame anc calcualte count from simulated "Data"
# last.ipm <- samp.ipm$N[,ncol(samp.ipm$N)]*fix.param$correction[[ncol(Data)]] # abundance on the last year
last.ipm <- samp.ipm$N[,j]*fix.param$correction[[j]]
...
samp.dist[,i] <- rnbinom(n = 1:nrow(Data), size = sample.cand[["nb.k"]], mu = last.ipm) #negative binomial
  ...
  if(!is.null( fix.param$ogive)){ #if has ogive function
  samp.dist.ogive <- samp.dist[c(fix.param$ogive:nrow(samp.dist)),] #only sizes that are observable
...
#integrate to bin
    samp.int.bin <- samp.bin %>%
      ...
      # mutate(sum.dist = sum.dist/fix.param$correction[[ncol(Data)]]) %>%
      mutate(sum.dist = sum.dist/fix.param$correction[[j]]) %>% #make abundance to density
...
# get quartile of each size
  ...
  #posterior interval - 95% and 50% interval
...
ppc.list[[paste0('pp.interval.year',j)]] <- samp.dist.quart
} #poster predictive check for last 3 years
  
  
# get quartile of each size in the observed data
obs.data <- list()

for (j in (ncol(Data)-2):ncol(Data)) { #get data for the last three years
  
  if(!is.null(fix.param$ogive)){ #get data, depending on if ogive function is given
    data.ipm <- Data[fix.param$ogive:nrow(Data),]

data.ipm <- data.ipm %>% select(j) %>% #select the data column (last 3 year of data)
  rename(count = everything()) %>% #rename so universial naming scheme 'count'
  ...
  mutate(length = as.numeric(c(1:10)),
         density = sum.bins/fix.param$correction[[j]]) # rename to have bins be numerical
  }else{
    data.ipm <- Data %>% select(j) %>% #select the data column (last 3 year of data)
      rename(count = everything()) %>% #rename so universial naming scheme 'count'
  ...
  obs.data[[paste0('data','.',"year",j)]] <- data.ipm
}
